# -*- coding: utf-8 -*-
"""
Created on Wed Sep  7 23:45:27 2016
...
#%% import data
data = pd.read_pickle('2013-12-12_Chosen_Popts.pandas')
...
def preprocess():
    ...
    for fluo in fluorophores:
        indexes[fluo] = [i for i in data.index if np.isfinite(data[fluo+'_chosen_popts'][i]).any()]
    
    #%% Correct popts
    Corrected_popts = {}
    for fluo in fluorophores:
        for parameter in sigmoid_parameters:
            Corrected_popts[fluo+'_'+parameter] = []
        
    for fluo in fluorophores:
        for i in data.index:
            if np.isfinite(data[fluo+'_chosen_popts'][i]).any():
                this_params = {}
                for popt in data[fluo+'_chosen_popts'][i]:
                    for this_popt, parameter in zip(popt, sigmoid_parameters):
                        this_params[parameter] = this_popt
                for parameter in sigmoid_parameters:
                    Corrected_popts[fluo+'_'+parameter].append(this_params[parameter])
            else:
                for parameter in sigmoid_parameters:
                    Corrected_popts[fluo+'_'+parameter].append(np.nan)
    
    for fluo in fluorophores:
        for parameter in sigmoid_parameters:
            data[fluo+'_'+parameter] = Corrected_popts[fluo+'_'+parameter]
    
    #%% Calculate and save normalized crossed intensities
    ...
    for fluo in fluorophores:
        ...
        for i in data.index:
            if i in indexes[fluo]:
                I_par_particular = af.I_par_FromAnisotropy(data['r_'+fluo][i], data['f_'+fluo][i])
                I_per_particular = af.I_per_FromAnisotropy(data['r_'+fluo][i], data['f_'+fluo][i])
                I_para.append(I_par_particular)
                I_perp.append(I_per_particular)
                I_para_r.append(I_par_particular/data['f_'+fluo][i])
                I_perp_r.append(I_per_particular/data['f_'+fluo][i])
                ...
                I_para.append(np.nan)
                I_perp.append(np.nan)
                I_para_r.append(np.nan)
                I_perp_r.append(np.nan)
        ...
        data[fluo+'_I_parallel'] = I_para
        data[fluo+'_I_perpendicular'] = I_perp
        data[fluo+'_I_parallel_n'] = I_para_r
        data[fluo+'_I_perpendicular_n'] = I_perp_r
...
if __name__ == '__main__':
    ...
    for b in bs:
        for experiment in experiments['YFP']:
            ...
            Analyzed.to_pickle(PREFIX + '_2013-12-12_'+experiment+'_b_'+str(int(b*100))+'.pandas')